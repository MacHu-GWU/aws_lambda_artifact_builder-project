...
def upload_layer_zip_to_s3(
    ...
    **Example:**
    ...
    .. code-block:: python
        ...
        # Configure S3 client and target directory
        s3_client = boto3.client("s3")
        s3dir_lambda = S3Path("s3://my-bucket/lambda/")
    ...
    # Initialize path management systems for local and S3 file organization
    # LayerPathLayout manages local build directory structure and file locations
    layer_path_layout = LayerPathLayout(
        path_pyproject_toml=path_pyproject_toml,
    )
    # LayerS3Layout manages S3 directory structure and naming conventions
    layer_s3_layout = LayerS3Layout(
        s3dir_lambda=s3dir_lambda,
    )
    # Determine source and target paths for the upload operation
    # Source: Local zip file created by the packaging step
    path_layer_zip = layer_path_layout.path_build_lambda_layer_zip
    # Target: S3 path following organized layer storage structure
    s3path = layer_s3_layout.s3path_temp_layer_zip
    ...
    s3path.upload_file(path=path_layer_zip, overwrite=True, bsm=s3_client)
